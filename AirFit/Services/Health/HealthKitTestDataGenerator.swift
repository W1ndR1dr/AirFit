...
final class HealthKitTestDataGenerator {
    ...
    func generateNutritionData(for date: Date) async throws {
        ...
        try await generateMeal(
            ...
            sodium: Double.random(in: 500...1000)
        )
    ...
    func generateSleepData(for date: Date) async throws {
        ...
        while currentTime < wakeTime.addingTimeInterval(-600) { // Wake up 10 minutes before alarm
            let sleepDuration = Double.random(in: 2700...5400) // 45-90 minutes
            ...
            if Double.random(in: 0...1) < 0.2 {
                let remDuration = Double.random(in: 600...1800) // 10-30 minutes
            ...
            if Double.random(in: 0...1) < 0.3 {
                let deepDuration = Double.random(in: 900...2700) // 15-45 minutes
            ...
            // Brief awake period
            if currentTime < wakeTime.addingTimeInterval(-1800) { // Not in the last 30 minutes
