...
final class PersonaSystemIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        unifiedAIService = await UnifiedAIService(apiKeyManager: apiKeyManager)
        
        // Initialize conversation manager
        let conversationManager = ConversationManager(modelContext: modelContext)
        
        // Initialize persona synthesizer
    ...
    func testCompleteOnboardingToCoachFlow() async throws {
        ...
        // Simulate orchestrator completing synthesis
        let persona = try await orchestrator.generatePersona(
        ...
        // Step 5: Save persona
        try await orchestrator.savePersona(persona, for: user.id)
    ...
    func testConversationRecovery() async throws {
        ...
        await orchestrator.pauseConversation()
        ...
        // Resume
        try await orchestrator.resumeConversation()
        XCTAssertEqual(orchestrator.state, .conversationInProgress)
    }
    ...
    func testMultiProviderFallback() async throws {
        ...
        await unifiedAIService.setFallbackProviders([.openAI, .googleGemini])
        ...
        let response = try await unifiedAIService.complete(request)
    ...
    func testSystemWideCaching() async throws {
        ...
        let response1 = try await unifiedAIService.complete(request)
        let duration1 = CFAbsoluteTimeGetCurrent() - start1
        
        // Second request (should hit cache)
        let start2 = CFAbsoluteTimeGetCurrent()
        let response2 = try await unifiedAIService.complete(request)
        ...
        // Check cache stats
        let stats = await unifiedAIService.getCacheStats()
    ...
    private func createRealisticInsights() -> PersonalityInsights {
        PersonalityInsights(
            ...
            motivationType: .extrinsic, // Wedding deadline
...
extension OnboardingOrchestrator {
    /// Test helper to generate persona directly
    func generatePersona(
        from data: ConversationData,
        insights: PersonalityInsights
    ) async throws -> PersonaProfile {
        // This would normally happen internally
        return try await personaSynthesizer.synthesizePersona(
            from: data,
            insights: insights
        )
    }
    
    /// Test helper to save persona
    func savePersona(_ persona: PersonaProfile, for userId: UUID) async throws {
        // This would normally happen as part of the flow
        state = .saving
        // Save logic here
        state = .completed
    }
