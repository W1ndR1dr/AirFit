...
final class HealthKitManagerTests: XCTestCase {
    ...
    func test_saveNutritionToHealthKit_withNilValues_handlesGracefully() async throws {
        ...
        // Assert
        XCTAssertTrue(success)
        XCTAssertNotEqual(nutrition.calories, 100, "Should not return hardcoded 100 calories")
    }
    
    func test_saveNutritionToHealthKit_whenErrorOccurs_throwsError() async throws {
        // Arrange
        let nutrition = NutritionData(calories: 200)
        mockHealthKitManager.shouldThrowError = true
    ...
    func test_syncFoodEntryToHealthKit_withNewEntry_createsHealthKitRecords() async throws {
        // Arrange
        let foodEntry = FoodEntry(date: Date(), user: testUser)
        ...
        foodEntry.foodItems = [foodItem]
        modelContext.insert(foodEntry)
        modelContext.insert(foodItem)
        try modelContext.save()
        
        mockHealthKitManager.stubbedResults["syncFoodEntryToHealthKit"] = ["sample-id-1", "sample-id-2"]
        
        // Act
        let sampleIDs = try await mockHealthKitManager.syncFoodEntryToHealthKit(foodEntry)
        
        // Assert
        XCTAssertFalse(sampleIDs.isEmpty)
        XCTAssertTrue(foodEntry.healthKitSynced)
        XCTAssertNotNil(foodEntry.healthKitSyncDate)
    }
    ...
    func test_syncFoodEntryToHealthKit_withExistingEntry_updatesHealthKitRecords() async throws {
        ...
        foodEntry.healthKitSynced = true
        ...
        mockHealthKitManager.stubbedResults["syncFoodEntryToHealthKit"] = ["new-sample-1", "new-sample-2"]
        
        // Act
        let newSampleIDs = try await mockHealthKitManager.syncFoodEntryToHealthKit(foodEntry)
        
        // Assert
        XCTAssertNotEqual(newSampleIDs, existingSampleIDs)
    ...
    func test_deleteFoodEntryFromHealthKit_withValidSampleIDs_deletesSuccessfully() async throws {
        ...
        // Assert
        XCTAssertTrue(success)
        XCTAssertFalse(foodEntry.healthKitSynced)
        XCTAssertTrue(foodEntry.healthKitSampleIDs.isEmpty)
    }
    ...
    func test_bulkNutritionSave_performance() async throws {
        ...
        let nutritionEntries = (0..<10).map { _ in
            NutritionData(
                ...
                carbs: Double.random(in: 20...100),
                fat: Double.random(in: 5...30)
            )
        }
        
        mockHealthKitManager.stubbedResults["saveNutritionToHealthKit"] = true
        mockHealthKitManager.simulatedDelay = 0.1 // 100ms per save
    ...
    func test_saveNutrition_withZeroCalories_savesSuccessfully() async throws {
        // Arrange
        let nutrition = NutritionData(calories: 0)
        mockHealthKitManager.stubbedResults["saveNutritionToHealthKit"] = true
    ...
    func test_concurrentNutritionSaves_preventDuplicates() async throws {
        // Arrange
        let nutrition = NutritionData(calories: 400, protein: 20, carbs: 40, fat: 15)
        mockHealthKitManager.stubbedResults["saveNutritionToHealthKit"] = true
        
        // Act - Save same nutrition data concurrently
        async let save1 = mockHealthKitManager.saveNutritionToHealthKit(nutrition, date: Date())
        async let save2 = mockHealthKitManager.saveNutritionToHealthKit(nutrition, date: Date())
        async let save3 = mockHealthKitManager.saveNutritionToHealthKit(nutrition, date: Date())
