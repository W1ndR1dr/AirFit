...
final class ContextAssemblerTests: XCTestCase {
    ...
    func test_assembleSnapshot_withStaleData_includesTimestampWarnings() async throws {
        ...
        staleMeal.timestamp = calendar.date(byAdding: .day, value: -2, to: Date())!
    ...
    func test_assembleSnapshot_withLargeDataSets_maintainsPerformance() async throws {
        ...
        for day in 0..<800 {
            ...
            if day < 30 {
                ...
                meal.timestamp = date
                let item = FoodItem(name: "Lunch Item \(day)", calories: Int.random(in: 200...600))
                meal.addItem(item)
                context.insert(meal)
            }
        }
    ...
    func test_assembleSnapshot_coordinating_HealthKitAndSwiftData_successfully() async throws {
        ...
        XCTAssertTrue(snapshot.appContext.lastMealSummary.contains("2 items"))
    ...
    func test_assembleSnapshot_withConcurrentAccess_remainsThreadSafe() async throws {
        ...
        // Act - run multiple snapshot assemblies concurrently
        await withTaskGroup(of: HealthContextSnapshot.self) { group in
            // Launch 10 concurrent snapshot assemblies
            for i in 0..<10 {
                group.addTask { [weak self] in
                    ...
                    return await self.sut.assembleSnapshot(modelContext: self.context)
                }
            }
