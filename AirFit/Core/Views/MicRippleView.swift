...
struct AudioLevelVisualizer: View {
    ...
    let audioLevel: Double // 0.0 to 1.0
    let barCount: Int
    ...
    private func barHeight(for index: Int) -> CGFloat {
        ...
        // Create wave effect
        let offset = Double(index) / Double(barCount - 1)
        let wave = sin(offset * .pi)
        
        let height = baseHeight + (maxHeight - baseHeight) * audioLevel * wave
