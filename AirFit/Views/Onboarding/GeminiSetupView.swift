...
struct GeminiSetupView: View {
    ...
    var body: some View {
        ZStack {
            BreathingMeshBackground(scrollProgress: 1.8)
                .ignoresSafeArea()
            ...
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 16) {
                        ...
                        Text("Add your Gemini API key to enable AI features.\n\nGet a free key from Google AI Studio.")
                            .font(.bodyMedium)
                    ...
                    VStack(spacing: 20) {
                                        ...
                                        RoundedRectangle(cornerRadius: 12)
                                            .strokeBorder(
                                                connectionStatus == .failed ? Theme.error :
                                                    ...
                                                    isKeyFieldFocused ? Theme.accent : Theme.textMuted.opacity(0.2),
                                                lineWidth: 1
                                            )
                                    )
                        ...
                        HStack(spacing: 8) {
                            Image(systemName: "info.circle")
                                .foregroundStyle(Theme.textMuted)
                            Text("Your API key is stored securely in Keychain and never sent anywhere except Google.")
                                .font(.caption)
                                .foregroundStyle(Theme.textMuted)
                        }
                        .padding(.top, 8)
        ...
        .onAppear {
            ...
            Task {
                if let existingKey = await keychainManager.getGeminiAPIKey() {
                    await MainActor.run {
                        apiKey = existingKey
                    }
                }
    ...
    private func testAPIKey() {
        ...
        Task {
            do {
                ...
                try await keychainManager.setGeminiAPIKey(apiKey)
...
#Preview {
    GeminiSetupView(onComplete: {}, onSkip: {})
}
